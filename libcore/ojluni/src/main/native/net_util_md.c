...
NET_MapSocketOption(jint cmd, int *level, int *optname) {
    static struct {
        ...
        { java_net_SocketOptions_SO_SND_TIMEOUT,        SOL_SOCKET,     SO_SNDTIMEO },
        { java_net_SocketOptions_SO_TIMEOUT,            SOL_SOCKET,     SO_RCVTIMEO },
    };
...
#define IPTOS_PREC_MASK 0xe0
    ...
    if (level == SOL_SOCKET) {
        if (opt == SO_RCVTIMEO || opt == SO_SNDTIMEO) {
          ...
          value.tv_sec = interval < 1000 ? 0 : ((interval / 1000) - (interval % 1000));
          value.tv_usec = interval < 1000 ? interval : (interval - (value.tv_sec * 1000));

          int ret = setsockopt(fd, level, opt, &value, sizeof(value));
          return ret;
        }
    }
