...
static int keymaster_create_key(struct crypt_mnt_ftr *ftr)
{
    ...
    if (keymaster_init(&keymaster0_dev, &keymaster1_dev)) {
        SLOGE("Failed to init keymaster");
        write_file_bootprof("vold:keymaster_create_key:END fail-1");
        return -1;
    }
    ...
    if (keymaster1_dev) {
        ...
        if (error != KM_ERROR_OK) {
            ...
            write_file_bootprof("vold:keymaster_create_key:END fail-2");
            rc = -1;
            goto out;
        }
        ...
        if (keymaster0_dev->generate_keypair(keymaster0_dev, TYPE_RSA, &params,
            ...
            write_file_bootprof("vold:keymaster_create_key:END fail-3");
            rc = -1;
            goto out;
        }
    } else {
        SLOGE("Cryptfs bug: keymaster_init succeeded but didn't initialize a device");
        write_file_bootprof("vold:keymaster_create_key:END fail-4");
        rc = -1;
        goto out;
    }
    ...
    if (key_size > KEYMASTER_BLOB_SIZE) {
        ...
        write_file_bootprof("vold:keymaster_create_key:END fail-5");
        rc = -1;
        goto out;
    }
                            ...
                            unsigned char *ikey, void *params)
{
    ...
    if (rc) {
        SLOGE("scrypt failed");
        write_file_bootprof("vold:scrypt_keymaster:scrypted fail-1");
        return -1;
    }
    write_file_bootprof("vold:scrypt_keymaster:scrypted");

    if (keymaster_sign_object(ftr, ikey, KEY_LEN_BYTES + IV_LEN_BYTES,
                              &signature, &signature_size)) {
        SLOGE("Signing failed");
        write_file_bootprof("vold:scrypt_keymaster:keymastered fail-2");
        return -1;
    }
    write_file_bootprof("vold:scrypt_keymaster:keymastered");
    ...
    if (rc) {
        SLOGE("scrypt failed");
        write_file_bootprof("vold:scrypt_keymaster:scrypted fail-3");
        return -1;
    }
                              ...
                              struct crypt_mnt_ftr *crypt_ftr)
{
    ...
    case KDF_SCRYPT_KEYMASTER:
        if (keymaster_create_key(crypt_ftr)) {
            ...
            write_file_bootprof("vold:encrypt_master_key:END fail-1");
            return -1;
        }

        if (scrypt_keymaster(passwd, salt, ikey, crypt_ftr)) {
            SLOGE("scrypt failed");
            write_file_bootprof("vold:encrypt_master_key:END fail-2");
            return -1;
        }
        break;
    ...
    case KDF_SCRYPT:
        if (scrypt(passwd, salt, ikey, crypt_ftr)) {
            SLOGE("scrypt failed");
            write_file_bootprof("vold:encrypt_master_key:END fail-3");
            return -1;
        }
        break;
    ...
    default:
        ...
        write_file_bootprof("vold:encrypt_master_key:END fail-4");
        return -1;
    }
    ...
    if (! EVP_EncryptInit_ex(&e_ctx, EVP_aes_128_cbc(), NULL, ikey, ikey+KEY_LEN_BYTES)) {
        ...
        write_file_bootprof("vold:encrypt_master_key:END fail-5");
        return -1;
    }
    ...
    if (! EVP_EncryptUpdate(&e_ctx, encrypted_master_key, &encrypted_len,
        ...
        write_file_bootprof("vold:encrypt_master_key:END fail-6");
        return -1;
    }
    if (! EVP_EncryptFinal_ex(&e_ctx, encrypted_master_key + encrypted_len, &final_len)) {
        SLOGE("EVP_EncryptFinal failed\n");
        write_file_bootprof("vold:encrypt_master_key:END fail-7");
        return -1;
    }

    if (encrypted_len + final_len != KEY_LEN_BYTES) {
        SLOGE("EVP_Encryption length check failed with %d, %d bytes\n", encrypted_len, final_len);
        write_file_bootprof("vold:encrypt_master_key:END fail-8");
        return -1;
    }
    ...
    */
    ...
    if (rc) {
      ...
      write_file_bootprof("vold:encrypt_master_key:END fail-9");
      return -1;
    }
                                  ...
                                  size_t* intermediate_key_size)
{
  ...
  if (kdf(passwd, salt, ikey, kdf_params)) {
    SLOGE("kdf failed");
    write_file_bootprof("vold:decrypt_master_key:END fail-1");
    return -1;
  }
  ...
  if (! EVP_DecryptInit_ex(&d_ctx, EVP_aes_128_cbc(), NULL, ikey, ikey+KEY_LEN_BYTES)) {
    write_file_bootprof("vold:decrypt_master_key:END fail-2");
    return -1;
  }
  ...
  if (! EVP_DecryptUpdate(&d_ctx, decrypted_master_key, &decrypted_len,
    ...
    write_file_bootprof("vold:decrypt_master_key:END fail-3");
    return -1;
  }
  if (! EVP_DecryptFinal_ex(&d_ctx, decrypted_master_key + decrypted_len, &final_len)) {
    write_file_bootprof("vold:decrypt_master_key:END fail-4");
    return -1;
  }

  if (decrypted_len + final_len != KEY_LEN_BYTES) {
    write_file_bootprof("vold:decrypt_master_key:END fail-5");
    return -1;
  }
...
int cryptfs_restart(void)
{
    ...
    ret = cryptfs_restart_internal(1);
...
int cryptfs_enable(char *howarg, int type, char *passwd, int no_ui)
{
    ...
    ret = cryptfs_enable_internal(howarg, type, passwd, no_ui);
    write_file_bootprof("vold:cryptfs_enable_internal:END");
    return ret;
}
...
int cryptfs_enable_default(char *howarg, int no_ui)
{
    int ret = 0;
    write_file_bootprof("vold:cryptfs_enable_internal:START");
    ret = cryptfs_enable_internal(howarg, CRYPT_TYPE_DEFAULT,
...
{
    //write_file_bootprof("vold:mountdefaultencrypted:START");
    ...
    if (crypt_type < 0 || crypt_type > CRYPT_TYPE_MAX_TYPE) {
        ...
        write_file_bootprof("vold:cryptfs_restart_internal:END");
        //write_file_bootprof("vold:mountdefaultencrypted:END");
        return 0;
    } else {
                           ...
                           unsigned char* master_key)
{
    ...
    write_file_bootprof("vold:get_master_key:START");
    ...
    if (rc) {
        ...
        write_file_bootprof("vold:get_master_key:END fail-1");
        return rc;
    }
    write_file_bootprof("vold:get_master_key:decrypted");
    ...
    if (rc) {
        ...
        write_file_bootprof("vold:get_master_key:END fail-2");
        return rc;
    }
    write_file_bootprof("vold:get_master_key:END");
