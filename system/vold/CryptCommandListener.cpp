...
void CryptCommandListener::dumpArgs(int argc, char **argv, int argObscure) {
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (((unsigned long)(p - buffer) + len) < (sizeof(buffer)-1)) {
            if (i == argObscure) {
                *p++ = '{';
                *p++ = '}';
                *p++ = ' ';
...
int CryptCommandListener::CryptfsCmd::runCommand(SocketClient *cli,
    ...
    if (subcommand == "checkpw") {
        ...
        if (!check_argc(cli, subcommand, argc, 2, "")) return 0;
        write_file_bootprof("vold:init_user0:START");
        rc = e4crypt_init_user0();
        write_file_bootprof("vold:init_user0:END");
        return sendGenericOkFailOnBool(cli, rc);
