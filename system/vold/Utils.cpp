...
status_t ForceUnmount(const std::string& path) {
    ...
    //sleep(5);
    /*  For some service, it will be re-started and access the storage again after killing it.
        In order to umount the storage well, vold will try to umount immediately.
    */
    for (int i = 0; i < 40; i++) {
        usleep(50*1000);
        if (!umount2(cpath, UMOUNT_NOFOLLOW) || errno == EINVAL || errno == ENOENT) {
