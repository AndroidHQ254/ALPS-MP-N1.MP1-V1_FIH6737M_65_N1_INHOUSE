...
static android::status_t mountInNamespace(uid_t uid, int device_fd, const std::string& path) {
    ...
    const char *cstr_opts, *cstr_path;
    ...
    const auto opts = android::base::StringPrintf(
            ...
            uid,
            uid);
    SLOGD("@@mountInNamespace opts: --");

    cstr_opts = opts.c_str();
    SLOGD("@@mountInNamespace cstr_opts: %s --", cstr_opts);

    cstr_path = path.c_str();
    SLOGD("@@mountInNamespace cstr_path: %s --", cstr_path);
...
*/
    for (int i = 0; i < 5; i++) {
        SLOGD("@@mountInNamespace mount --");
        if (mount("/dev/fuse", cstr_path, "fuse",
            MS_NOSUID | MS_NODEV | MS_NOEXEC | MS_NOATIME, cstr_opts) == 0) {
            SLOGI("%s sucessfully mounted ", cstr_opts);
            return android::OK;
        }
        sleep(1);
