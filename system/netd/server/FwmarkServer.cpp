...
#define LOG_TAG "FwmarkServer"
#include <sys/socket.h>
#include <unistd.h>
#include <cutils/log.h>
#include <string.h>

FwmarkServer::FwmarkServer(NetworkController* networkController, NetlinkManager* netlinkManager) :
        SocketListener("fwmarkd", true), mNetworkController(networkController), mNetlinkManager(netlinkManager) {
}
...
int FwmarkServer::processClient(SocketClient* client, int* socketFd) {
    ...
    if(int ret = mNetworkController->mobileDataControl(client->getUid(), command.netId)) {
        ALOGI("client uid %ld, ret %d", (long)(client->getUid()), ret);
        sprintf(buf, "%ld mobile data blocking", (long)(client->getUid()));
        mNetlinkManager->getBroadcaster()->sendBroadcast(ResponseCode::StrictSocketConn, buf, false);
        return ret;
    }
    ...
    switch (command.cmdId) {
        ...
        case FwmarkCommand::SELECT_NETWORK: {
            ...
            if (command.netId == NETID_UNSET) {
                ...
                if (int ret = mNetworkController->checkUserNetworkAccess(client->getUid(),
                                                                         command.netId)) {
                    ALOGI("SELECT_NETWORK, uid %d cannot access network %u, %s", client->getUid(), command.netId, strerror(-ret));
                    return ret;
                }
        ...
        case FwmarkCommand::PROTECT_FROM_VPN: {
            if (!mNetworkController->canProtect(client->getUid())) {
                ALOGI("PROTECT_FROM_VPN, uid %d cannot protect from vpn", client->getUid());
                return -EPERM;
            }
        ...
        case FwmarkCommand::SELECT_FOR_USER: {
            if ((permission & PERMISSION_SYSTEM) != PERMISSION_SYSTEM) {
                ALOGI("SELECT_FOR_USER, system permission are needed");
                return -EPERM;
            }
    ...
    if(LOG_DBG) ALOGI("socket %d, uid %u, netid %u, fmwark.intValue %d", *socketFd, client->getUid(), command.netId, fwmark.intValue);
    return 0;
}
