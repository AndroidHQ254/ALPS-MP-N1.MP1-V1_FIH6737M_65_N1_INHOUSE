...
unsigned char * formatTxtRecord(int txtLen, unsigned char *lastTxtRecord, unsigned char *ptrTxtRecord);

static MDnsSdListener *sListener;
static void timer_handle(int sig);

void timer_handle(int sig) {
    ALOGD("[timer_handle] sig = %d\n", sig);

    if (sListener) {
        sListener->close_timer();
        char *msg;
        asprintf(&msg, "%d %d", sListener->mTimer_RefNumber, kDNSServiceErr_Unknown);
        sListener->sendBroadcast(ResponseCode::ServiceResolveFailed, msg, false);
        ALOGE("[timer_handle] resolve failure for %d, error= %d", sListener->mTimer_RefNumber, kDNSServiceErr_Unknown);
        free(msg);
    }
}
...
void MDnsSdListener::open_timer(int requestId) {
   ALOGD("[open_timer] 1s\n");

    if (sListener)
        sListener->mTimer_RefNumber = requestId;

    struct itimerval timer_tick;
    timer_tick.it_value.tv_sec = 1;
    timer_tick.it_value.tv_usec = 0;
    timer_tick.it_interval.tv_sec = 1;
    timer_tick.it_interval.tv_usec = 0;

    int timer_ret = setitimer(ITIMER_REAL, &timer_tick, NULL);
    if (timer_ret != 0) {
        ALOGD("[open_timer] error: %s\n", strerror(errno));
    }
}

void MDnsSdListener::close_timer() {
    ALOGD("[close_timer]\n");
    struct itimerval timer_tick;
    timer_tick.it_value.tv_sec = 0;
    timer_tick.it_value.tv_usec = 0;
    timer_tick.it_interval = timer_tick.it_value;
...
void MDnsSdListener::Handler::resolveService(SocketClient *cli, int requestId,
    ...
    mListener->open_timer(requestId);
...
void MDnsSdListenerResolveCallback(DNSServiceRef /* sdRef */, DNSServiceFlags /* flags */,
    ...
    if (context->mRefNumber != context->mListener->mTimer_RefNumber) {
        ALOGE("previous refnumber, has expired, ignored it. curr_number = %d; callback_unmber = %d",
               context->mListener->mTimer_RefNumber, context->mRefNumber);
        return;
    }
    context->mListener->close_timer();
    ...
    if (errorCode != kDNSServiceErr_NoError) {
        ...
        /*parsing txtRecord content and format it for nsdservice*/
        unsigned char lastTxtRecord[2048] = "";
        txtRecord = formatTxtRecord(txtLen, lastTxtRecord, (unsigned char *)txtRecord);
        asprintf(&msg, "%d %s %s %d %d %s %s", refNumber, quotedFullName, quotedHostTarget, port, txtLen, dst, txtRecord);
        ...
        if (VDBG) {
            ALOGD("resolve succeeded for %d finding %s at %s:%d with txtLen %d %s",
                    refNumber, fullname, hosttarget, port, txtLen, txtRecord);
        }
    }
    free(msg);
}
...
unsigned char * formatTxtRecord(int txtLen, unsigned char *lastTxtRecord, unsigned char *srcTxtRecord) {
    ALOGE("[formatTxtRecord] txtLen = %d; lastTxtRecord = %p; ptrTxtRecord = %s", txtLen, lastTxtRecord, srcTxtRecord);
    if (txtLen > 0 && srcTxtRecord) {
        unsigned char *ptrDes = lastTxtRecord;
        unsigned char *ptrSrc = srcTxtRecord;
        int readCount = 0;
        int txtCount = 0;
        while (readCount < txtLen) {
            txtCount = *ptrSrc;
            ptrSrc ++;
            ALOGE("[formatTxtRecord] txtCount = %d; readCount = %d", txtCount, readCount);
            if (readCount + txtCount + 1 > txtLen) {
                ALOGE("[formatTxtRecord] error: readCount(%d) > txtLen(%d)", readCount, txtLen);
                break;
            }
            readCount += (txtCount + 1);
            *ptrDes ++ = '"';
            memcpy((char*)ptrDes, (char*)ptrSrc, txtCount);
            ptrDes += txtCount;
            *ptrDes ++ = '"';
            if (readCount < txtLen) {
                *ptrDes ++ = ' ';
        ...
        if (readCount == txtLen) {
            ALOGE("[formatTxtRecord] format txtRecord completed: lastTxtRecord = %s", lastTxtRecord);
            return lastTxtRecord;
        }
    }
    return srcTxtRecord;  //format txtRecord fail
}
