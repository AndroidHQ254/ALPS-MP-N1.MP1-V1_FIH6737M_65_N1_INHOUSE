...
static int execIptablesCommand(int argc, const char *argv[], bool silent) {
    ...
    if(LOG_DBG) ALOGI("execIptablesCommand android_fork_execvp enter");
    res = android_fork_execvp(argc, (char **)argv, &status, false,
        !silent);
    if(LOG_DBG) ALOGI("execIptablesCommand android_fork_execvp exit");
