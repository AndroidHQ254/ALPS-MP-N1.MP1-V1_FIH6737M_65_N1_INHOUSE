...
static bool script_run_once = false;
void setupAppIptablesHook() {
    const char *file_path = "/data/misc/dhcp/fw_iptables.conf"; 
    if (!script_run_once && 0 == access(file_path, F_OK)) {
        char iptables_cmd[128];
        FILE *f = fopen(file_path, "r");
        if (!f) {
            ALOGE("open file:%s failed for reading", file_path);
            return;
        }
        memset(iptables_cmd, 0, sizeof(iptables_cmd));
        while(fgets(iptables_cmd, sizeof(iptables_cmd), f) != NULL){
            int len = strlen(iptables_cmd);
            /*delete the last '\n'*/
            if(iptables_cmd[len-1] == '\n')
                iptables_cmd[len-1] = '\0';
            else
                ALOGW("%s is not end with enter", iptables_cmd);
                
            ALOGD("run cmd: %s", iptables_cmd);
            system_nosh(iptables_cmd);
            memset(iptables_cmd, 0, sizeof(iptables_cmd));
        }
        fclose(f);
        ...
        if(!script_run_once)
            ALOGW("can't access iptables config file");
    }
}
