...
// M: if eager reader read in-bound data failed, we want caller to process
// the fail case.
// If not set eager_reader_inbound_fail_cb or eager_reader_inbound_fail_cb
// cannot handle the fail case, the default process is used, that is to print
// a warning log and continue polling in-bound data
typedef bool (*eager_reader_read_inbound_fail_cb)(int error_code);
void eager_reader_set_read_fail_cb(eager_reader_t *reader,
    eager_reader_read_inbound_fail_cb read_fail_cb);
