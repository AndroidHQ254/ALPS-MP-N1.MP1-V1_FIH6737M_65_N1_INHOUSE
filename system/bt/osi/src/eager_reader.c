...
struct eager_reader_t {
  ...
  // M: if eager reader read fails, dispatch this info to its callbacks
  // in order to let its callbacks do some extra action accordingly.
  eager_reader_read_inbound_fail_cb inbound_data_read_fail_cb;
};
...
static void inbound_data_waiting(void *context) {
  ...
  if (bytes_read > 0) {
    ...
    if (bytes_read == 0)
      ...
      if (reader->inbound_data_read_fail_cb != NULL
        && !reader->inbound_data_read_fail_cb(errno))
...
void eager_reader_set_read_fail_cb(eager_reader_t *reader,
  ...
  reader->inbound_data_read_fail_cb = read_fail_cb;
}
