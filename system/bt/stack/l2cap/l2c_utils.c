...
BOOLEAN l2cu_set_acl_priority (BD_ADDR bd_addr, UINT8 priority, BOOLEAN reset_after_rs)
{
    ...
    /* modify there for change the acl buffer for profile connection */
    ...
    if (p_ccb->buff_quota != 0)
    {
        ...
        if ( p_ccb->cong_sent )
        {
            ...
            if (q_count <= (p_ccb->buff_quota / 2))
            {
                ...
                {
                    L2CAP_TRACE_DEBUG ("L2CAP - Calling CongestionStatus_Cb (FALSE), CID: 0x%04x  xmit_hold_q._count: %u  buff_quota: %u",
