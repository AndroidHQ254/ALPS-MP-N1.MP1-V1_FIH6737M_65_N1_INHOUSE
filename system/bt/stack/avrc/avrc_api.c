                               ...
                               BT_HDR *p_pkt)
{
    ...
    if (p_pkt->layer_specific & AVCT_DATA_CTRL) {
        ...
        msg.hdr.ctype = cr;
        AVRC_TRACE_DEBUG("avrc_msg_cback - browse: handle: %d, ctype: %d, offset: %d, len: %d", handle, cr, p_pkt->offset, p_pkt->len);
...
#if (MTK_AVRCP_TG_15 == TRUE)
    else if (p_pkt->event == AVRC_OP_BROWSE)
    {
        p_pkt->offset -= AVRC_MIN_BROWSE_HDR_SIZE;
        p_pkt->len += AVRC_MIN_BROWSE_HDR_SIZE;
    }
#endif
