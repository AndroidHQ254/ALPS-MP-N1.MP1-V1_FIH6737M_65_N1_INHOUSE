...
**
** Description      Open L2CAP channel to peer for browse
**
**
** Returns          Nothing.
**
...
void avct_bcb_chnl_open(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_API("avct_bcb_chnl_open");

    /* call l2cap connect req */
    p_bcb->ch_state = AVCT_CH_CONN;

    if ((p_bcb->ch_lcid = L2CA_ConnectReq(AVCT_BR_PSM, p_data->p_ccb->p_lcb->peer_addr)) == 0) {
        /* if connect req failed, send ourselves close event */
        AVCT_TRACE_ERROR("avct_bcb_chnl_open: fail");
...
**
** Description      do on avct_lcb_unbind_disc
**
**
** Returns          Nothing.
**
...
**
** Description      Handle an LL_OPEN event.  For each allocated ccb already
**                  bound to this bcb, send a connect event.  For each
**                  unbound ccb, bind that ccb to this bcb and
**                  send a connect event.
**
**
** Returns          Nothing.
...
void avct_bcb_open_ind(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data)
{
    ...
    AVCT_TRACE_API("avct_bcb_open_ind");
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
        /* if ccb allocated and */
        if (p_ccb->allocated & AVCT_ALOC_BCB) {
            /* if bound to this lcb send connect confirm event */
            if (p_ccb->p_bcb == p_bcb) {
               	AVCT_TRACE_DEBUG("avct_bcb_open_ind: AVCT_BROWSE_CONN_CFM_EVT");
                bind = TRUE;
                p_ccb->cc.p_ctrl_cback(avct_ccb_to_idx(p_ccb), AVCT_BROWSE_CONN_CFM_EVT, 0, p_ccb->p_lcb->peer_addr);
            } else if (p_ccb->p_bcb == NULL) { /* if unbound acceptor */
               	AVCT_TRACE_DEBUG("avct_bcb_open_ind: AVCT_BROWSE_CONN_IND_EVT");
                /* bind ccb to lcb and send connect ind event */
                bind = TRUE;
                p_ccb->p_bcb = p_bcb;
                p_ccb->cc.p_ctrl_cback(avct_ccb_to_idx(p_ccb), AVCT_BROWSE_CONN_IND_EVT, 0, p_ccb->p_lcb->peer_addr);
            }
        }
    }
    ...
    /* if no ccbs bound to this lcb, disconnect */
    if (bind == FALSE) {
        AVCT_TRACE_WARNING("avct_bcb_open_ind: no bcb allocated with ccb");
...
**
** Description      L2CAP channel open attempt failed.  Deallocate any ccbs
**                  on this bcb and send connect confirm event with failure.
**
**
** Returns          Nothing.
**
...
void avct_bcb_open_fail(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
        if ((p_ccb->allocated & AVCT_ALOC_BCB) && (p_ccb->p_bcb == p_bcb)) {
            AVCT_TRACE_DEBUG("avct_bcb_open_fail: bcb, lcid", p_bcb->ch_lcid);
            memset(p_bcb, 0, sizeof(tAVCT_BCB));
            p_ccb->p_bcb = NULL;
            p_ccb->allocated &= ~AVCT_ALOC_BCB;
            (*p_ccb->cc.p_ctrl_cback)(avct_ccb_to_idx(p_ccb), AVCT_BROWSE_CONN_CFM_EVT, p_data->result, p_ccb->p_lcb->peer_addr);
        }
    }
}
...
**
** Description      L2CAP channel closed by peer.  Deallocate any initiator
**                  ccbs on this bcb and send disconnect ind event.
**
**
** Returns          Nothing.
**
...
void avct_bcb_close_ind(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
        if ((p_ccb->allocated & AVCT_ALOC_BCB) && (p_ccb->p_bcb == p_bcb)) {
            AVCT_TRACE_DEBUG("avct_bcb_close_ind: bcb lcid", p_bcb->ch_lcid);
            memset(p_bcb, 0, sizeof(tAVCT_BCB));
            p_ccb->p_bcb = NULL;
            p_ccb->allocated &= ~AVCT_ALOC_BCB;
            (*p_ccb->cc.p_ctrl_cback)(avct_ccb_to_idx(p_ccb), AVCT_BROWSE_DISCONN_IND_EVT, 0, p_ccb->p_lcb->peer_addr);
        }
    }
}
...
**
** Description      L2CAP channel closed by us.  Deallocate any initiator
**                  ccbs on this bcb and send disconnect ind or cfm event.
**
**
** Returns          Nothing.
**
...
void avct_bcb_close_cfm(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
        if ((p_ccb->allocated & AVCT_ALOC_BCB) && (p_ccb->p_bcb == p_bcb)) {
            AVCT_TRACE_DEBUG("avct_bcb_close_cfm: bcb lcid", p_bcb->ch_lcid);
            memset(p_bcb, 0, sizeof(tAVCT_BCB));
            p_ccb->p_bcb = NULL;
            p_ccb->allocated &= ~AVCT_ALOC_BCB;
            (*p_ccb->cc.p_ctrl_cback)(avct_ccb_to_idx(p_ccb), AVCT_BROWSE_DISCONN_CFM_EVT,0, p_ccb->p_lcb->peer_addr);
        }
    }
}
...
**
** Description      Bind ccb to bcb and send connect cfm event.
**
**
** Returns          Nothing.
**
...
void avct_bcb_bind_conn(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_API("avct_bcb_bind_conn");
    p_data->p_ccb->p_bcb = p_bcb;
    (*p_data->p_ccb->cc.p_ctrl_cback)(avct_ccb_to_idx(p_data->p_ccb),AVCT_BROWSE_CONN_CFM_EVT, 0, p_data->p_ccb->p_lcb->peer_addr);
}
...
**
** Description     do as lcb_chk_disc
**
**
** Returns          Nothing.
**
...
void avct_bcb_chk_disc(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    AVCT_TRACE_API("avct_bcb_chk_disc");
    AVCT_TRACE_WARNING("avct_bcb_chk_disc: do as lcb_chk_disc");
}
...
void avct_bcb_chnl_disc(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_DEBUG("avct_bcb_chnl_disc: bcb lcid: %x", p_bcb->ch_lcid);
...
void avct_bcb_bind_fail(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_DEBUG("avct_bcb_bind_fail: bcb lcid: %x", p_bcb->ch_lcid);
...
void avct_bcb_cong_ind(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_DEBUG("avct_bcb_cong_ind, evet: %x", event);
    p_bcb->cong = p_data->cong;

    if (p_bcb->cong == FALSE && fixed_queue_try_peek_first(p_bcb->tx_q)) {
        while ( !p_bcb->cong  && (p_buf = (BT_HDR *) fixed_queue_dequeue(p_bcb->tx_q)) != NULL) {
            if (L2CA_DataWrite(p_bcb->ch_lcid, p_buf) == L2CAP_DW_CONGESTED)
                p_bcb->cong = TRUE;
        }
    }

    /* send event to all ccbs on this bcb */
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
        if ((p_ccb->allocated & AVCT_ALOC_BCB)&& (p_ccb->p_bcb == p_bcb)) {
            (*p_ccb->cc.p_ctrl_cback)(avct_ccb_to_idx(p_ccb), event, 0, p_ccb->p_lcb->peer_addr);
        }
    }
}
...
void avct_bcb_send_msg(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    if (curr_msg_len <= (p_bcb->peer_mtu - AVCT_HDR_LEN_SINGLE)) {
        pkt_type = AVCT_PKT_TYPE_SINGLE;
        p_buf = p_data->ul_msg.p_buf;
        p_buf->offset -= AVCT_PKT_TYPE_SINGLE;
        p_buf->len += AVCT_PKT_TYPE_SINGLE;
        p = (UINT8 *)(p_buf + 1) + p_buf->offset;
        ...
        } else { /* send message to L2CAP */
            if (L2CA_DataWrite(p_bcb->ch_lcid, p_buf) == L2CAP_DW_CONGESTED)
                p_bcb->cong = TRUE;
        }
    } else {
        AVCT_TRACE_ERROR("avct bcb lenth incorrect");
    }

    AVCT_TRACE_DEBUG ("avct_bcb_send_msg tx_q_count:%d", fixed_queue_length(p_bcb->tx_q));
...
**
** Description      Discard an incoming AVCTP browse message.
**
**
** Returns          Nothing.
**
...
void avct_bcb_free_msg_ind(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_API("avct_bcb_free_msg_ind");
...
**
** Description      Deallocate a link browse block.
**
**
** Returns          void.
**
...
void avct_bcb_dealloc(tAVCT_BCB *p_bcb, tAVCT_LCB_EVT *p_data){
    ...
    AVCT_TRACE_API("avct_bcb_dealloc");

    if (p_bcb == NULL) {
        AVCT_TRACE_WARNING("avct_bcb_dealloc: NULL bcb");
        return;
    }
    ...
    if (!found) {
        ...
        if (p_bcb->p_tx_msg != NULL)
            osi_free(p_bcb->p_tx_msg);

        memset(p_bcb, 0, sizeof(tAVCT_LCB));
    }
}
#endif
