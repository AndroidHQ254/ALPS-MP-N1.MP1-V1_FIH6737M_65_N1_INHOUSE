...
**
** Description      find the bcb corresponding lcb and disconnect it
**
**
** Returns          Nothing.
**
...
void avct_close_bcb(tAVCT_LCB *p_lcb, tAVCT_LCB_EVT *p_data){
    tAVCT_CCB *p_ccb = p_data->p_ccb;
    int i;

    AVCT_TRACE_DEBUG("avct_close_bcb");

    if (p_ccb == NULL) {
        AVCT_TRACE_DEBUG("avct_close_bcb: ccb not found");
        return;
    }

    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++){
        if ((p_ccb->allocated & AVCT_ALOC_BCB )&& (p_ccb->p_lcb == p_lcb)) {
            if (p_ccb->p_bcb != NULL) {
                AVCT_TRACE_DEBUG("avct_close_bcb: found bcb, disconnect it");
                p_ccb->allocated &= ~AVCT_ALOC_BCB;
                avct_bcb_event(p_ccb->p_bcb, AVCT_LCB_INT_CLOSE_EVT, p_data);
                fixed_queue_free(p_ccb->p_bcb->tx_q, NULL);
                p_ccb->p_bcb = NULL;
            }
        }
    }
...
tAVCT_LCB *avct_lcb_by_bcb(tAVCT_BCB *p_bcb){
    ...
    if (p_bcb != NULL) {
        if ((index = avct_bcb_to_idx(p_bcb))< AVCT_NUM_LINKS) {
            p_lcb = &avct_cb.lcb[index];

            if (p_lcb->allocated != 0)
                return p_lcb;

            AVCT_TRACE_DEBUG("avct_lcb_by_bcb: no lcb for this bcb: %d", index);
        } else
            AVCT_TRACE_DEBUG("avct_lcb_by_bcb: bcb out of range");
    } else
        AVCT_TRACE_DEBUG("avct_lcb_by_bcb: bcb is NULL");
...
tAVCT_BCB *avct_bcb_by_lcb(tAVCT_LCB *p_lcb){
    ...
    if (p_lcb != NULL) {
        if ((index = avct_lcb_to_idx(p_lcb))< AVCT_NUM_LINKS) {
            p_bcb = &avct_cb.bcb[index];

            if (p_bcb->allocated != 0)
                return p_bcb;

            AVCT_TRACE_DEBUG("avct_bcb_by_lcb: no bcb for this lcb: %d", index);
        } else
            AVCT_TRACE_DEBUG("avct_bcb_by_lcb: bcb out of range");
    } else
        AVCT_TRACE_DEBUG("avct_bcb_by_lcb: lcb is NULL");
...
**
*******************************************************************************/
BOOLEAN avct_bcb_last_ccb(tAVCT_BCB *p_bcb, tAVCT_CCB *p_ccb_last){
    tAVCT_CCB   *p_ccb = &avct_cb.ccb[0];
    int         i;

    AVCT_TRACE_API("avct_bcb_last_ccb");

    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
        if (p_ccb->allocated && (p_ccb->p_bcb == p_bcb) && (p_ccb != p_ccb_last)) {
            return FALSE;
        }
    }
...
tAVCT_BCB *avct_bcb_by_lcid(UINT16 lcid){
    ...
    for (i = 0; i < AVCT_NUM_LINKS; i++) {
        if (avct_cb.bcb[i].ch_lcid == lcid)
            return &avct_cb.bcb[i];
    }

    AVCT_TRACE_DEBUG("no bcb for this lcid: %d", lcid);
...
/*******************************************************************************
**
** Function         avct_bcb_to_idx
**
** Description      return bcb index on acvt_cb.bcb[]
**
**
** Returns
**
*******************************************************************************/
UINT8 avct_bcb_to_idx(tAVCT_BCB *p_bcb)
{
    /* use array arithmetic to determine index */
    return (UINT8) (p_bcb - avct_cb.bcb);
}
#endif
#endif
...
/*******************************************************************************
**
** Function         avct_lcb_to_idx
**
** Description      return lcb index on acvt_cb.lcb[]
**
**
** Returns
**
*******************************************************************************/
UINT8 avct_lcb_to_idx(tAVCT_LCB *p_lcb)
{
    /* use array arithmetic to determine index */
    return (UINT8) (p_lcb - avct_cb.lcb);
}
