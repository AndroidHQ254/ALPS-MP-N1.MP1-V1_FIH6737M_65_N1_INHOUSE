...
static bool hal_open() {
  ...
  eager_reader_set_read_fail_cb(uart_stream, handleFirmwareReset);
#else
  eager_reader_set_read_fail_cb(uart_stream, NULL);
#endif
  ...
  *(data) = type;
  ...
  while (length > 0) {
    ...
    switch (ret) {
      case -1:
#if (MTK_SUPPORT_FW_CORE_DUMP == TRUE)
        if (handleFirmwareReset(errno)) {
          // M: do nothing, and waiting chip reset complete
        } else
#endif
