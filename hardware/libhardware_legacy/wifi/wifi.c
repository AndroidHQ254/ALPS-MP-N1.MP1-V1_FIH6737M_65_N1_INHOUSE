...
void wifi_enable_debug_log(int more_log)
{
    if (more_log)
        property_set("log.tag.WifiHW", "D");
    else
        property_set("log.tag.WifiHW", "W");
}
...
#ifdef CONFIG_MTK_NOT_ENG_LOAD
        wifi_enable_debug_log(0);
#endif
        return 0;
    }
...
#ifdef CONFIG_MTK_NOT_ENG_LOAD
    cmd = strstr(cmd, "LOG_LEVEL ");
    if (cmd) {
        if (strncmp(cmd+10, "DEBUG", 5) == 0)
            wifi_enable_debug_log(1);
        else
            wifi_enable_debug_log(0);
    }
